...
def calculate_amplitudes(raw_adc_values, sampling_rate):
    """Calculates the peak-to-peak amplitude for the raw and filtered signals."""
    print("[CLIENT] Calculating signal amplitudes...")
    ...
    try:
        ...
        dc_removed_signal = remove_dc_offset_temp(raw_adc_values)
        ...
        if filtered_ac_signal.size > 0 and not np.all(np.isnan(filtered_ac_signal)):
            dc_component = np.mean(np.array(raw_adc_values, dtype=np.float32))
            reconstructed_filtered_signal = filtered_ac_signal + dc_component
        ...
        return raw_amplitude, filtered_amplitude
    except Exception as e:
        print(f"[CLIENT] Error during amplitude calculation: {e}")
        return None, None

# --- RECTIFIED: This function now saves data to a unique file for each analysis ---
def save_amplitude_results(original_file_name, raw_amplitude, filtered_amplitude):
    ...
    try:
        ...
        # Create a new filename based on the original filename
        output_filename = f"analysis_{original_file_name}"
        # Use os.path.join for cross-platform compatibility
        output_filepath = os.path.join(output_directory, output_filename)
        
        # Use 'w' (write mode) to create a new file for each analysis
...
def remove_dc_offset_temp(values):
    ...
    try:
        ...
        print(f"[CLIENT] Error in remove_dc_offset_temp: {e}")
...
def init_plot(label="ADC Data"):
    """Initializes the Matplotlib figure and axes for 2 plots."""
    global current_fig, current_axes, current_raw_line, current_filtered_line
    with plot_lock:
        plt.close('all')
        current_fig, current_axes = plt.subplots(2, 1, figsize=(10, 6), sharex=True)
        current_fig.subplots_adjust(hspace=0.4)
...
def process_and_plot_live_data(raw_adc_values, interval_ms, file_name):
    """Calculates amplitudes, saves them, then simulates live plotting."""
    if not raw_adc_values:
    ...
    # Call the analysis functions once per file
    raw_amp, filtered_amp = calculate_amplitudes(raw_adc_values, sampling_rate)
    save_amplitude_results(file_name, raw_amp, filtered_amp)

    # Continue with the live simulation as before
    ...
    for adc_value in raw_adc_values:
        ...
        if len(dsp_raw_adc_buffer) >= min_dsp_samples:
            ...
            processed_window = remove_dc_offset_temp(dsp_window)
            ...
            if filtered_ac.size > 0 and not np.all(np.isnan(filtered_ac)):
                dc_component = np.mean(dsp_window)
                reconstructed_signal = filtered_ac[-1] + dc_component
...
def receive_data_loop(data_queue):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if not config_len_bytes: raise ConnectionError("Server disconnected before sending config.")
            config_len = int.from_bytes(config_len_bytes, 'big')
            config_data_bytes = recvall(s, config_len)
            if not config_data_bytes: raise ConnectionError("Server disconnected before sending config data.")
            ...
            for line in config_data.split('\\n'):
                if "INTERVAL:" in line:
                    try:
                        ...
                        print(f"[CLIENT] Set interval from server: {interval_ms} ms")
        ...
        while not shutdown_event.is_set():
            try:
                ...
                if data_packet is None:
                    ...
                    break
                raw_adc_values, interval_ms, file_name = data_packet
                current_raw_buffer.clear()
                current_filtered_buffer.clear()
                dsp_raw_adc_buffer.clear()
                process_and_plot_live_data(raw_adc_values, interval_ms, file_name)
            except queue.Empty:
                continue
            except Exception as e:
