...
def fir_filter(values, cut_off_frequency, sampling_rate):
    ...
    try:
        ...
        # --- FINAL CORRECTION: Removed the status check entirely. ---
        # The modern Pythonic way is to assume success if no exception is raised.
...
def update_live_plot():
    ...
    if shutdown_event.is_set() or current_fig is None:
        return
    with plot_lock:
        try:
...
def process_and_plot_live_data(raw_adc_values, interval_ms, file_name):
    """Simulates live processing and plotting of data from a full file."""
    if not raw_adc_values:
        print(f"[CLIENT] No data to process for {file_name}.")
        return

    print(f"[CLIENT] Simulating live processing for {file_name}...")
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if not config_len_bytes:
                raise ConnectionError("Server disconnected before sending config.")
                
            config_len = int.from_bytes(config_len_bytes, 'big')
            config_data_bytes = recvall(s, config_len)
            if not config_data_bytes:
                 raise ConnectionError("Server disconnected before sending config data.")
