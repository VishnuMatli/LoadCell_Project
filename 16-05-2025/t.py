...
def live_plot(interval_ms, label="ADC Data"):
    ...
    def update_plot(new_raw_values, new_filtered_values):
        nonlocal x_data, raw_y_data, filtered_y_data, fft_x_data, fft_y_data, start_time, sampling_rate

        if not new_raw_values or not new_filtered_values:
            return  # Check for empty data

        for i in range(len(new_filtered_values)):
            x_data.append(len(x_data))  # Use len(x_data) for continuous x-axis
            raw_buffer.append(new_raw_values[i])
            filtered_buffer.append(new_filtered_values[i])
            ...
            ax1.set_xlim(0, max(100, len(x_data) + 10))
            ax2.set_xlim(0, max(100, len(x_data) + 10))

            elapsed_time = time.time() - start_time
            actual_sampling_rate = len(filtered_y_data) / elapsed_time if elapsed_time > 0 else sampling_rate
            sampling_rate = actual_sampling_rate # update
    ...
    plt.tight_layout()
    return update_plot, fig # Return the update function and the figure
...
def process_data(raw_values, interval_ms, file_name, update_plot_func):
    """
    ...
    Args:
        ...
        update_plot_func: the plot update function.
    """
    try:
        if len(raw_values) > 0:
            ...
            fig = update_plot_func(normalized_raw_values, normalized_filtered_values) # Get figure
            if fig:
                fig.canvas.draw()
...
def receive_and_plot():
    ...
    try:
        ...
        mode = "interval"
        file_name = ""
        update_plot, fig = live_plot(interval_ms) # Initialize the plot and get the update function.
                    ...
                    for line in lines:
                        if 'ADC:' in line:
                            try:
                                ...
                                pass
                    if raw_values:
                        # Pass the update_plot function to process_data
                        threading.Thread(target=process_data, args=(raw_values, interval_ms, file_name, update_plot)).start()
                    else:
